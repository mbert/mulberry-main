...
CAddressTableNewAction::CAddressTableNewAction(CAddressTable* itsTable,
												CAdbkAddress* addr,
												bool wasDirty) :
		LAction(STRx_RedoAddressActions, str_AddressNew, false)
{
	mItsTable = itsTable;
...
void CAddressTableNewAction::AddAddresses(CAddressList* new_addrs)
{
	...
	for(CAddressList::iterator iter = new_addrs->begin(); iter != new_addrs->end(); iter++)
	{
		...
		if (!mNewItems->IsDuplicate(*iter))
			...
			// Must delete those not being used
			delete *iter;
	}
}

void CAddressTableNewAction::GetDescription(Str255 outRedoString, Str255 outUndoString) const
{
	LStr255 redo(rsrc::GetString("UI::Adbk::AddressNew_Redo"));
	LStr255 undo(rsrc::GetString("UI::Adbk::AddressNew_Undo"));
...
void CAddressTableEditAction::GetDescription(Str255 outRedoString, Str255 outUndoString) const
{
	LStr255 redo(rsrc::GetString("UI::Adbk::AddressEdit_Redo"));
	LStr255 undo(rsrc::GetString("UI::Adbk::AddressEdit_Undo"));
...
void CAddressTableDeleteAction::GetDescription(Str255 outRedoString, Str255 outUndoString) const
{
	LStr255 redo(rsrc::GetString("UI::Adbk::AddressDelete_Redo"));
	LStr255 undo(rsrc::GetString("UI::Adbk::AddressDelete_Undo"));
