...
class CPeriodicCheck : public LPeriodical
{
public:
					CPeriodicCheck()
						{ mLastSpendTime = 0; mBlock = false;
							mPwrSignal = false; mPwrSignalProcess = false; mCallDepth = 0; };
...
private:
	...
	bool			mBlock;
	bool			mPwrSignal;
	bool			mPwrSignalProcess;
	unsigned long	mCallDepth;

	void			CheckAppIdleTooLong();
};

class CPeriodicReset : public LAttachment
{
public:
	...
	virtual bool	UserPaused(void);

protected:
	virtual void	ExecuteSelf(MessageT inMessage,
								void* ioParam);				// Reset if key or mouse event
private:
	time_t			mLastTime;
	time_t			mLastSpendTime;
