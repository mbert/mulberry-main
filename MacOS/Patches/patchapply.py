...
def applypatch(project, patch):
    stat = os.system("%s -s -d \"%s/\" -p0 --forward --dry-run -i %s > /dev/null" % (cmd, project[0], patch, ))
    if stat == 0:
        print "+++ Patching \"%s\" with %s" % (project[0], patch[len(cwd) + 1:],)
        os.system("%s -s -d \"%s/\" -p0 --forward -i %s" % (cmd, project[0], patch, ))
    else:
        print "*** Failed to patch \"%s\" with %s" % (project[0], patch[len(cwd) + 1:],)
