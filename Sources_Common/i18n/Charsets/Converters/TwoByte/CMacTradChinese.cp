...
// Source for CMacTradChinese class

#include "CMacTradChinese.h"

using namespace i18n;

const size_t cTableOffset1 = 0xA1;
const size_t cTableOffset21 = 0x40;
const size_t cTableOffset22 = 0xA1;
const size_t cTableSize1 = 89;
const size_t cTableSize21 = 63;
const size_t cTableSize22 = 94;
const size_t cTableMax1 = cTableOffset1 + cTableSize1;
const size_t cTableMax21 = cTableOffset21 + cTableSize21;
const size_t cTableMax22 = cTableOffset22 + cTableSize22;

struct SBig5Planes
{
  wchar_t plane0[cTableSize21];
  wchar_t plane1[cTableSize22];
};

const SBig5Planes cTableData[cTableSize1] =
{
	{ // Index 0x01  Page 0xA100
		{
			0x3000, 0xFF0C, 0x3001, 0x3002, 0xFF0E, 0x00B7, 0xFF1B, 0xFF1A, 0xFF1F, 0xFF01,
			0xFE30, 0x22EF, 0x2025, 0xFF0C, 0x3001, 0xFF0E, 0xFF0E, 0xFF1B, 0xFF1A, 0xFF1F,
			0xFF01, 0xFF5C, 0x2013, 0xFE31, 0x2014, 0xFE33, 0xFF3F, 0xFE34, 0xFE4F, 0xFF08,
			...
			0xFE44, 0xFF08, 0xFF09
		},
		{
			0xFF5B, 0xFF5D, 0x3014, 0x3015, 0x2018, 0x2019, 0x201C, 0x201D, 0x301D, 0x301E,
			...
			0x25BC, 0x32A3, 0x2105, 0x203E, 0x203E, 0xFF3F, 0xFF3F, 0xFE49, 0xFE4A, 0xFE4D,
			0xFE4E, 0xFE4B, 0xFE4B, 0xFE5F, 0xFE60, 0xFE61, 0xFF0B, 0xFF0D, 0x00D7, 0x00F7,
			...
			0x2642, 0x2295, 0x2609, 0x2191, 0x2193, 0x2190, 0x2192, 0x2196, 0x2197, 0x2199,
			0x2198, 0x2225, 0x2223, 0xFF0F
		}
	},
	{ // Index 0x02  Page 0xA200
		{
			0xFF3C, 0xFF0F, 0xFF3C, 0xFF04, 0x00A5, 0x3012, 0x00A2, 0x00A3, 0xFF05, 0xFF20,
			...
			0x2534, 0x252C, 0x2524, 0x251C, 0x2594, 0x2500, 0x2502, 0x2502, 0x250C, 0x2510,
	...
	{ // Index 0x03  Page 0xA300
		...
		},
		{
			...
			0x02CB, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000,
	...
	{ // Index 0x59  Page 0xF900
		...
		},
		{
			...
			0x9F7E, 0x9F49, 0x9F98, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000,
			0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000,
			0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000,
			0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000, 0x0000,
			0x0000, 0x0000, 0x0000, 0x0000
...
wchar_t CMacTradChinese::c_2_w(const unsigned char*& c)
{
	unsigned char c1 = *c++;
	...
	// Look for other one-byte characters
	else
	{
		// Check Mac OS specific mappings
		if (c1 == 0x80)
			return 0x005C;
		else if (c1 == 0x81)
			return 0xF880;
...
static const size_t num_pages = 8;
static const wchar_t page_bounds[num_pages][2] =
{
	{0x0000, 0x0100},
	{0x0200, 0x03D0},
	{0x2000, 0x22F0},
...
static const char* page_map[num_pages] = {NULL};
...
int CMacTradChinese::w_2_c(wchar_t wc, char* out)
{
	...
	else
	{
		...
		{
			if ((wc >= page_bounds[i][0]) && (wc < page_bounds[i][1]))
			{
				unsigned long index = 2 * (wc - page_bounds[i][0]);
				*out++ = page_map[i][index];
				*out = page_map[i][index + 1];
				return 2;
			}
		}
	...
	// If we get here its because we processed a one-byte character
	return 1;
}

void CMacTradChinese::init_tables()
{
	// Intitialise each table
	for(size_t i = 0; i < num_pages; i++)
		page_map[i] = new char[2 * (page_bounds[i][1] - page_bounds[i][0])];

	// Loop over each jis->unicode map point and add to appropriate table
	for(unsigned long i = 0; i < cTableSize1; i++)
	{
		// plane0
		for(unsigned long j = 0; j < cTableSize21; j++)
		{
			// Detrmine the unicode char
			wchar_t wc = cTableData[i].plane0[j];
			
			// Get the jis char
			char char1 = i + cTableOffset1;
			char char2 = j + cTableOffset21;

			// Fill in unicode map entry
			for(size_t k = 0; k < num_pages; k++)
			{
				if ((wc >= page_bounds[k][0]) && (wc < page_bounds[k][1]))
				{
					unsigned long index = 2 * (wc - page_bounds[k][0]);
					const_cast<char*>(page_map[k])[index] = char1;
					const_cast<char*>(page_map[k])[index + 1] = char2;
				}
			}
		}
		
		// plane1
		for(unsigned long j = 0; j < cTableSize22; j++)
		{
			// Detrmine the unicode char
			wchar_t wc = cTableData[i].plane1[j];
			
			// Get the jis char
			char char1 = i + cTableOffset1;
			char char2 = j + cTableOffset22;
