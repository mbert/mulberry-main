...
// 15-Sep-2002 cd: move into Sources_Common and switch to use new CLetterWindow apis
//
...
//
// 	Unix:
// This function is called from the unix signal handler. It
// must therefore protect aganst any kind of additional signals/exceptions (Unix or C++)
// to prevent total failure of the exception handler.
//
// History:
// 11-Sep-2002 cd: created initial implementation
// 15-Sep-2002 cd: new CLetterWindow api
//
...
bool CSafetySave::RecoverDraft(const cdstring& path)
{
	...
	{
		// Now open the draft
		result = CLetterWindow::OpenDraft(path.c_str());
