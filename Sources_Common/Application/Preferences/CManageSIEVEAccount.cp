...
#pragma mark ____________________________CManageSIEVEAccount

CManageSIEVEAccount::CManageSIEVEAccount()
{
	// Start with anonymous authenticator
	GetAuthenticator().ResetAuthenticatorType(CAuthenticator::ePlainText);
}

void CManageSIEVEAccount::_copy(const CManageSIEVEAccount& copy)
{
	mFuture = copy.mFuture;
}

// Compare with same type
int CManageSIEVEAccount::operator==(const CManageSIEVEAccount& comp) const
{
	return CINETAccount::operator==(comp);
}

cdstring CManageSIEVEAccount::GetInfo(void) const
{
...
bool CManageSIEVEAccount::SetInfo(char_stream& txt, NumVersion vers_prefs)
{
	bool result = true;
