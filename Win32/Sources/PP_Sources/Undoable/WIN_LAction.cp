...
LAction::GetDescription(
	CString&	outRedoString,
	CString&	outUndoString) const
{
	if (mStringIndex == 0)
	{
		if (mStringR.GetLength() != 0)
			outRedoString = mStringR;
		else
			outRedoString = rsrc::GetString("UI::General::Redo");
		if (mStringU.GetLength() != 0)
	...
	{
		outRedoString.LoadString(mStringIndex);
		outUndoString.LoadString(mStringIndex + 1);
	}
}
