...
// Source for CTitleTableViewView class
...
BEGIN_MESSAGE_MAP(CTitleTableView, CTable)
	ON_WM_CREATE()
	ON_WM_LBUTTONDOWN()
	ON_WM_LBUTTONUP()
	ON_WM_RBUTTONDOWN()
...
HCURSOR CTitleTableView::sVertCursor = NULL;

const SInt32 cSizeSlop = 3;
...
int CTitleTableView::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	while(parent && !dynamic_cast<CTableView*>(parent))
		parent = parent->GetParent();
	mTableView = dynamic_cast<CTableView*>(parent);
...
void CTitleTableView::SetTitleInfo(bool can_sort, bool can_size, const char* rsrcid, UINT max_str, UINT menu_id)
{
	mCanSort = can_sort;
	mCanSize = can_size;
	mMenuID = menu_id;
...
// Clicked somewhere
void CTitleTableView::OnRButtonDown(UINT nFlags, CPoint point)
{
	STableCell	hitCell;
	CPoint		imagePt;
	...
	if (popup.LoadMenu(mMenuID))
	{
		...
		if (result)
		{
			...
			default:
				...
				mTableView->SetColumnType(hitCell.col, result - pPopup->GetMenuItemID(0) + 1);
				if (sort_col)
					mTableView->SetSortBy(result - pPopup->GetMenuItemID(0) + 1);
				break;
			}
		}
		RedrawWindow();
...
// Clicked somewhere
void CTitleTableView::OnLButtonDown(UINT nFlags, CPoint point)
{
	if (mSortTracking || mSizeTracking)
		return;

	// Check for possible track start
	if (mCanSort || mCanSize)
	{
		...
		if (GetCellHitBy(imagePt, cell))
		{
			...
			if (mCanSize && ((point.x < cellRect.left + cSizeSlop) && (cell.col > 1) ||
				(point.x > cellRect.right - cSizeSlop)))
			{
				mSizeTracking = true;

				// Determine which column to track
				if (point.x < cellRect.left + cSizeSlop)
				...
				CRect table;
				mTableView->mTable->GetWindowRect(table);
...
// Clicked somewhere
void CTitleTableView::OnLButtonUp(UINT nFlags, CPoint point)
{
	...
	if (mSizeTracking)
	{
		...
		if (moveby)
		{
			...
			if (new_width < 16)
				new_width = 16;
			mTableView->SetColumnWidth(mTrackingCol, new_width);
		}
...
void CTitleTableView::OnMouseMove(UINT nFlags, CPoint point)
{
	...
	else if (mCanSize)
	{
		...
		if (GetCellHitBy(imagePt, cell))
		{
			...
			// See if mouse is close to the left or right edges
			if ((point.x < cellRect.left + cSizeSlop) && (cell.col > 1) ||
				(point.x > cellRect.right - cSizeSlop))
			{
...
BOOL CTitleTableView::OnSetCursor(CWnd* pWnd, UINT nHitTest, UINT message)
{
...
void CTitleTableView::OnInvertTracker(const CRect& rect)
{
...
// Draw a row
void CTitleTableView::DrawCell(CDC* pDC, const STableCell& inCell, const CRect& inLocalRect)
{
	if (mTableView == NULL)
		return;
...
void CTitleTableView::DrawItem(CDC* pDC, SColumnInfo& col_info, const CRect &cellRect)
{
...
// Draw text
void CTitleTableView::DrawText(CDC* pDC, SColumnInfo& col_info, const CRect &cellRect)
{
...
// Draw icon
void CTitleTableView::DrawIcon(CDC* pDC, SColumnInfo& col_info, UINT iconID, const CRect &cellRect)
{
