...
BEGIN_MESSAGE_MAP(CDayWeekView, CCalendarViewBase)
	ON_WM_CREATE()
	...
	ON_COMMAND(IDC_DAYVIEW_WEEKPREV, OnPrevWeek)
	ON_COMMAND(IDC_DAYVIEW_WEEKNEXT, OnNextWeek)
	ON_COMMAND(IDC_DAYVIEW_DAYPREV, OnPrevDay)
	ON_COMMAND(IDC_DAYVIEW_DAYNEXT, OnNextDay)
	ON_COMMAND(IDC_DAYVIEW_TIMEZONE_POPUP, OnTimezone)
	ON_COMMAND(IDC_DAYVIEW_SCALE_POPUP, OnScale)
	ON_COMMAND(IDC_DAYVIEW_RANGE_POPUP, OnRange)
...
CDayWeekView::CDayWeekView(CCommander* cmdr) :
...
int CDayWeekView::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	CRect r = CRect(cWeekPrevBtnHOffset, cBtnVOffset, cWeekPrevBtnHOffset + cBtnSize, cBtnVOffset + cBtnSize);
	mPrevWeek.Create(_T(""), r, this, IDC_DAYVIEW_WEEKPREV, IDC_STATIC, IDI_PREVMSG);

	r = CRect(cDayPrevBtnHOffset, cBtnVOffset, cDayPrevBtnHOffset + cBtnSize, cBtnVOffset + cBtnSize);
	mPrevDay.Create(_T(""), r, this, IDC_DAYVIEW_DAYPREV, IDC_STATIC, IDI_PREVMSG);

	r = CRect(cDayNextBtnHOffset, cBtnVOffset, cDayNextBtnHOffset + cBtnSize, cBtnVOffset + cBtnSize);
	mNextDay.Create(_T(""), r, this, IDC_DAYVIEW_DAYNEXT, IDC_STATIC, IDI_NEXTMSG);

	r = CRect(cWeekNextBtnHOffset, cBtnVOffset, cWeekNextBtnHOffset + cBtnSize, cBtnVOffset + cBtnSize);
	mNextWeek.Create(_T(""), r, this, IDC_DAYVIEW_WEEKNEXT, IDC_STATIC, IDI_NEXTMSG);
	...
	mTimezonePopup.Create(_T(""), r, this, IDC_DAYVIEW_TIMEZONE_POPUP, IDC_STATIC, IDI_POPUPBTN);
	...
	mScalePopup.Create(_T(""), r, this, IDC_DAYVIEW_SCALE_POPUP, IDC_STATIC, IDI_POPUPBTN);
	...
	mRangePopup.Create(_T(""), r, this, IDC_DAYVIEW_RANGE_POPUP, IDC_STATIC, IDI_POPUPBTN);
	...
	mTable.Create(NULL, NULL, WS_CHILD | WS_VISIBLE | WS_TABSTOP | WS_VSCROLL,
					CRect(0, cTopHeight + cTitleHeight, width, height), this, IDC_DAYVIEW_TABLE);
...
{
	return const_cast<CDayWeekTable*>(&mTable);
}
...
{
	mType = type;
	mPrevWeek.ShowWindow(mType == eDay ? SW_SHOW : SW_HIDE);
	mNextWeek.ShowWindow(mType == eDay ? SW_SHOW : SW_HIDE);
}
...
{
	mDayWeekScale = scale;
	mScalePopup.SetValue(mDayWeekScale + IDM_DAYVIEW_SCALE_FIT);
}

void CDayWeekView::OnScale()
{
	mDayWeekScale = mScalePopup.GetValue() - IDM_DAYVIEW_SCALE_FIT;
...
{
	mDayWeekRange = range;
	mRangePopup.SetValue(mDayWeekRange + IDM_DAYVIEW_RANGE_FULL);
}

void CDayWeekView::OnRange()
{
	mDayWeekRange = static_cast<CDayWeekViewTimeRange::ERanges>(mRangePopup.GetValue() - IDM_DAYVIEW_RANGE_FULL);
	ResetDate();
	RedrawWindow();
}
